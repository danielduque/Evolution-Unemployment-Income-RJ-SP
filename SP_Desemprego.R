...
for (x in 1:8){
 ...
 # Calcular Renda Domiciliar per Capita e Taxa de Desemprego
  RTDOMpcmed <- svyratio(~RTD, ~n_ind, dstrat1, na.rm = T)
  ...
  # Criar base que será adicionada à SP
  ...
  #Jogar resultados anteriores na base SP
  
  SP <- rbind(SP,PNADC_T2)    
...
ggplot(SP,aes(x=AnoTri,group=Tipo))+
  ...
  labs(title="Desemprego em SP (em %)", caption="Fonte: PNAD Contínua 2ºTri",x="",y="",colour="")+
...
# Usar geobr para ler os municípios de SP
...
# Juntar SP2 com UF por código do município
...
plot1 <- ggplot() + geom_sf(data=SPfim, aes(fill= as.numeric(`Desemprego`)),
  ...
  labs(title="Desemprego em SP", subtitle = "AnoTri: {current_frame}", caption="Fonte: PNAD Contínua 2ºTri",x="",y="",fill="")+
  coord_sf(datum = NA) + 
  transition_manual(AnoTri)

# Segundo mapa-gif (Renda Domiciliar per Capita)
plot2 <- ggplot() + geom_sf(data=SPfim, aes(fill= as.numeric(`RTDpc Médio`)),
                            color = "grey", size = 0.0001) +
  scale_fill_distiller(palette = "Blues", 
                       breaks = pretty_breaks(n = 6),
                       direction=1)+
  guides(fill = guide_legend(reverse = TRUE))+
  labs(title="Renda do Trabalho Domiciliar Per Capita em SP", subtitle = "AnoTri: {current_frame}", caption="Fonte: PNAD Contínua 2ºTri",x="",y="",fill="")+
  coord_sf(datum = NA) + 
  transition_manual(AnoTri)


anim_save(plot1,file="Desemp_SP.gif")
anim_save(plot2,file="RTDPC_SP.gif")
